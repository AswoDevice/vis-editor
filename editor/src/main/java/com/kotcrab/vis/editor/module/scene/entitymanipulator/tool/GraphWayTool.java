...
import com.kotcrab.vis.editor.module.scene.action.ChangeGraphWayAction;
...
/** @author Kotcrab */
public class GraphWayTool extends BaseSelectionTool {
	public static final String TAG = "GraphWayTool";
	public static final String TOOL_ID = App.PACKAGE + ".tools.GraphWayTool";
	...
	private ChangeGraphWayAction changePolygonAction;
	...
	private void drawLine (Batch batch, float x1, float y1, float x2, float y2, int thickness) {
		...
		batch.draw(polygonDown, x1, y1, 0, 0, polygonDown.getRegionWidth(), polygonDown.getRegionHeight(), 1, 1, rad * MathUtils.radiansToDegrees);
	}
	...
	private void makeDefaultPolygon () {
		...
		ChangeGraphWayAction action = new ChangeGraphWayAction(entityManipulator, proxy);
	...
	public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
		...
		if (button == Buttons.LEFT && component != null) {
			if (lineOverStartVertex != null) {
				...
				changePolygonAction = new ChangeGraphWayAction(entityManipulator, proxy);
	...
	public void selectedEntitiesChanged () {
		...
		if (proxy.hasComponent(GraphWay.class) == false) {
		...
		component = proxy.getComponent(GraphWay.class);
